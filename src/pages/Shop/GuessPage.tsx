...
async function notifyCustomerJoin(
  roomId: number,
  customerId: number,
  customerName: string,
  phone: string
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!customerInfo.customerId)
        throw new Error("Không có thông tin khách hàng");

      sessionStorage.setItem("customerInfo", JSON.stringify(customerInfo));
      const roomId = parseInt(sessionStorage.getItem("roomId") || "0", 10);
      notifyCustomerJoin(
        roomId,
        Number(customerInfo.customerId),
        customerInfo.name,
        customerInfo.phone
      );
      navigate("/shop/menu");
      ...
      >
        ...
        >
          <motion.div
            whileTap={{ scale: 0.98 }}
            style={
              { position: "relative", marginBottom: "25px" } as MotionStyle
            }
          >
            <input
              type="tel"
              ...
              style={{
                ...
                borderRadius: "12px",
                border: `2px solid ${
                  phoneError ? "#d32f2f" : isPhoneFocused ? "#007bff" : "#ddd"
                }`,
                outline: "none",
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            disabled={
              !phoneNumber || (isNameDisabled ? false : !name) || isLoading
            }
            style={
              {
                width: "100%",
                ...
                color: "#fff",
                backgroundColor:
                  phoneNumber && (isNameDisabled || name)
                    ? isLoading
                      ? "#6c757d"
                      : "#007bff"
                    : "#aaa",
                ...
                borderRadius: "12px",
                cursor:
                  phoneNumber && (isNameDisabled || name) && !isLoading
                    ? "pointer"
                    : "not-allowed",
                transition: "all 0.3s ease",
                marginBottom: "20px",
